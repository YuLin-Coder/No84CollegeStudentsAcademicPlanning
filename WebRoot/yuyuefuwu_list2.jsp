...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="cccccc" >  
 ...
 <% 
    ...
    String url = "yuyuefuwu_list2.jsp?1=1"; 
    String sql =  "select * from yuyuefuwu where yuyueren='"+request.getSession().getAttribute("username")+"' ";
  ...
  <tr>
    ...
    <td><%=map.get("yuyuedanhao") %></td> <td><%=map.get("fuwumingcheng") %></td> <td><%=map.get("zhuanye") %></td> <td><%=map.get("leibie") %></td> <td><%=map.get("jiage") %></td> <td><%=map.get("yuyueshiduan") %></td> <td><%=map.get("yuyueren") %></td>  <td align='center'><%=map.get("iszf")%>  <% if(map.get("iszf").equals("否")){%><a class="btn btn-info btn-small" href="zhifu/index.jsp?id=<%=map.get("id")%>&biao=yuyuefuwu"><font color='red'>支付</font></a> <%}%> </td>    
    <td width="138" align="center"><%=map.get("addtime") %></td>
    <td width="220" align="center"><a class="btn btn-info btn-small" href="yuyuefuwu_updt.jsp?id=<%=map.get("id")%>">修改</a>  <a class="btn btn-info btn-small" href="yuyuefuwu_list2.jsp?scid=<%=map.get("id") %>" onClick="return confirm('真的要删除？')">删除</a> <a class="btn btn-info btn-small" href="yuyuefuwu_detail.jsp?id=<%=map.get("id")%>">详细</a> </td>
  </tr>
  	<%
  }
   %>
